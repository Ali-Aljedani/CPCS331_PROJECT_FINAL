...
def newdfs(maze: Maze, cell: Cell):
    ...
    path = [] # initial the path with empty
    ...
    while stack: # while stack is not empty then continue the loop
        ...
        if currentCell.x == maze.goal.x and currentCell.y == maze.goal.y: # if the currentCell position is the same as the goal one
            ...
            # if a node has no neighbors that mean we faced a dead end, then we should keep removing from our path until we find node that is a neighbor with our top of stack
            if(neighbors == []):
                while path:
                    if Maze.is_neighbor(maze,x1=path[-1][0], x2=stack[-1].x, y1=path[-1][1], y2=stack[-1].y): # Note: path[-1] is a tuble so numbers are used to access its elements
                        break # If we find that our top of path is a neighbor of our top of stack then break and do not pop
                    path.pop()
